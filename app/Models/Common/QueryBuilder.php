...
class QueryBuilder extends Builder
{
    ...
    public function __construct(ConnectionInterface $connection,
        	Grammar $grammar = null, Processor $processor = null )
    {
    	parent::__construct($connection, $grammar, $processor);
    
    	$this->meta = Container::getInstance()->make(IMeta::class);
    	$this->cache = Container::getInstance()->make(ICache::class);
    } 
    ...
    public function get($columns = ['*'])
    {
        ...
        $results = [];
        if ($this->isAwful()) {
            $results = $this->getAwful();
        } elseif ($this->isNormal()) {
            $results = $this->getAwful();
        } else {
            $results = $this->getSimple();
        }
    ...
    private function getSimple()
    {
        ...
        $cachedRows = $this->cache->get(array_values($cacheKeys));
    ...
    public function delete($id = null)
    {
        if ($this->needFlushCache()) {
            // 清空表级缓存
            $this->meta->flush($this->db(), $this->model->table());
    ...
    private function flushAffectingRowCache()
    {
        ...
        if ($ids) {
            $cacheKeys = $this->buildRowCacheKey($ids);
            $this->cache->del(array_values($cacheKeys));
        }
    }
    ...
    public function insert(array $values)
    {
        if ($this->needFlushCache()) {
            ...
            $this->cache->del(array_values($toClearKeys));
        }
    ...
    {
        $prefix = $this->meta->prefix($this->db(), $this->model->table());
    ...
    {
        $parts = [
            $this->meta->prefix($this->db(), $this->model->table(), true),
            $sql,
    ...
    {
        $this->meta->flushAll($this->db(), $this->model->table());
    }
